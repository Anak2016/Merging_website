...
<div id="ratingDetails">
	<div class="row">
		<div class="col-sm-3">
			...
			<h2 class="bold padding-buttom-7">{{round($avg,1)}}<small>/ 5</small></h2>
			<?php
	...
	<div class="row">
		<div class="col-sm-7">
			...
			<div class="row">
				<div class="col-sm-3">
					<img src='{{$item_rating['user']->image_path}}' class='img-responsive' >
					{{-- <img src="/sam_public/images/product_images/product_demo.jpg" class="img-responsive"> --}}

					<div class='review-block-name'>By <a href='/sam_public/customer'>{{$item_rating['user']->name}}</a></div>                                
					<div class="review-block-date">{{$item_rating['user']->created_at}}</div>
				</div>
				<div class="col-sm-9">
					<?php
					$ratingClass = "btn-success";
					...
					?>
					@for($i=1;$i<=5;$i++)
					@if($i <= $item_rating->rating_number )
					<button type="button" class="btn btn-sm <?php echo $ratingClass;?>" aria-label="Left Align">
						<span class="glyphicon glyphicon-star" aria-hidden="true"></span>
					</button>
					@else
					...
					@endif
					@endfor
					<div class="review-block-title">{{$item_rating->title}}</div>
					<div class="review-block-description">{{$item_rating->comments}}</div>
				</div>
		...
		<div class="col-sm-12">
			<form id="ratingForm" method="post" enctype="/sam_public/customer/comment">
				<div class="form-group">
					...
					<input type="hidden" class="form-control" id="pro_id" name="product_id" value="{{$product['id']}}">
